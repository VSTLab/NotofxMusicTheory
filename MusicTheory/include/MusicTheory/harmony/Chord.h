...
namespace MusicTheory {
	...
	static std::map<std::string, std::vector<std::shared_ptr<Chord>>> _triads_cache;

	//A cache for composed sevenths
	static std::map<std::string, std::vector<std::shared_ptr<Chord>>> _sevenths_cache;
	...
	typedef std::shared_ptr<Chord>(*ChordShorthandFuncPointer)(NotePtr);
	typedef std::map<std::string, ChordShorthandFuncPointer> ChordShorthandFuncLookup;



	class Chord : public std::enable_shared_from_this<Chord> {
	...
	public:
		...
		std::vector<std::shared_ptr<Chord> > polychords;

		Chord(std::string _name = "") {
			if (_name != "") {
				set(_name);
			}
		};
		...
		void set(std::string _name = "C") {
			std::shared_ptr<Chord> c = Chord::fromShorthand(_name);
			if (c) {
				if (c->isValid()) {
					name = c->name;
					notes = c->notes;
...
//===================================================================
	 ...
	 */
	 //alias
	 //Accepts C13
		static std::shared_ptr<Chord> getChordFromString(std::string shorthand_string) {
			return Chord::fromShorthand(shorthand_string);
		}
		//accepts 13 Note("C")
		static std::shared_ptr<Chord> getChordFromString(std::string shorthand_string, NotePtr note) {
		...
		static std::shared_ptr<Chord> fromShorthand(std::string shorthand_string) {
			//Shrink shorthand_string to a format recognised by chord_shorthand
			boost::replace_all(shorthand_string, "min", "m");
			boost::replace_all(shorthand_string, "mi", "m");
			boost::replace_all(shorthand_string, "-", "m");
			boost::replace_all(shorthand_string, "maj", "M");
			boost::replace_all(shorthand_string, "ma", "M");


			auto slash = utils::splitString(shorthand_string, "/");
			auto poly = utils::splitString(shorthand_string, "|");

			std::string top = shorthand_string;
			...
			std::string name = Chord::getRootNote(top);
			NotePtr note = Note::create(name);
			std::string chordSymbol = Chord::getChordSymbol(top);

			//this retrives the actual notes

			std::shared_ptr<Chord> chord = Chord::chordFromShorthand(chordSymbol, note);

			if (!chord) {
				return 0;
			}

			//cout<<"::fromShorthand chordSymbol "<<chordSymbol<<" "<<chord->getName()<<std::endl;
			...
			if (poly.size() == 2) {
				//get polychord
				std::string name = Chord::getRootNote(poly[1]);
				NotePtr note = NotePtr(new Note(name));
				std::string chordSymbol = Chord::getChordSymbol(poly[1]);//was top?
				std::shared_ptr<Chord> chord2 = Chord::chordFromShorthand(chordSymbol, note);
				std::shared_ptr<Chord> subchord = Chord::create();
		...
		static std::shared_ptr<Chord>create(std::string _name = "") {
			return std::shared_ptr<Chord>(new Chord(_name));
		}

		std::shared_ptr<Chord> copy() {
			std::shared_ptr<Chord>s = std::shared_ptr<Chord>(new Chord(*this));
			s->notes.clear();
			for (NotePtr n : notes) {
...
//===================================================================
	 ...
	 */
		 ...
		 */
		std::deque<NotePtr> getAllNotes() {
			std::deque<NotePtr> allNotes = notes;
			if (polychords.size() > 0) {
				//allNotes.insert(allNotes.cend(), polychords[0]->notes.begin(), polychords[0]->notes.end());
				allNotes.append_range(polychords[0]->notes);
			}
			return allNotes;
		}
		 ...
		 */
		 /* Note root(){
			  ...
			  if(root){
			  std::string rootStr = Chord::getRootNote(name);
			  cout<<"rootStr "<<rootStr<<std::endl;
			  NotePtr n(rootStr);
		...
		*/
...
#ifdef LOGS
				ofLogError() << "Chord is missing notes" << std::endl;
#endif // LOGS
				return Note::create();
			}
		...
		std::shared_ptr<Chord> getTransposed(int interval) {
			std::shared_ptr<Chord> copy = this->copy();
			copy->transpose(interval);
			return copy;
		}


		void setPolyChord(std::shared_ptr<Chord> pc) {
			std::shared_ptr<Chord> subchord = Chord::create();
		...
		std::string getChordSymbol() {
			...
			// cout<<"Chord::getChordSymbol for "<<name<<" "<<nm<<std::endl;
			std::string str = Chord::getChordSymbol(nm);
			return str;
		}
...
//===================================================================
   ...
   */
		bool isMajor() {
			...
			std::shared_ptr<Chord> copy = rootPosition();
...
//===================================================================
   ...
   */
		...
		std::shared_ptr<Chord> findNearestVoicing(std::shared_ptr<Chord> chord) {
		...
		*/
		...
		std::shared_ptr<Chord> findNearestVoicing(std::shared_ptr<Note> note) {
		...
		*/
		...
		std::shared_ptr<Chord> findNearestVoicingBelow(std::shared_ptr<Note> note, bool okToOverlap = true) {
		...
		std::shared_ptr<Chord> findNearestVoicingAbove(std::shared_ptr<Note> note, bool okToOverlap = true) {
		...
		*/
		...
		std::shared_ptr<Chord> findNearestTransposition(std::shared_ptr<Note> note) {
...
//===================================================================
	...
	//this is ok to check for empty pointers
		static bool isValid(std::shared_ptr<Chord>c) {
			if (!c) {
				return false;
			}
			else {
		...
		static bool isValidName(std::string str) {
			std::string name = ChordLookup[str];
			if (name.size() > 0) {
				return true;
			}
			else {
		...
		static std::string getFullName(std::string str) {
			std::string name = ChordLookup[str];//map entries without key instantiates to empty std::string
			if (name.size() > 0) {
				return name;
			}
			else {
		...
		static std::vector<std::string> getAllKnownChords() {
			static std::vector<std::string> _allchords;
		...
		static std::vector<std::string> getAllMajorChords() {
			static std::vector<std::string> _allmajchords;

			if (_allmajchords.size() == 0) {
				typedef std::map<std::string, std::string>::iterator it_type;
				for (it_type iterator = ChordLookup.begin(); iterator != ChordLookup.end(); iterator++) {
					std::shared_ptr<Chord>chord = Chord::create("C" + iterator->first);
					if (chord->isMajor()) {
		...
		static std::vector<std::string> getAllMinorChords() {
			static std::vector<std::string> _allminchords;
		...
		static std::vector<std::string> getAllDominantChords() {
			static std::vector<std::string> _alldomchords;
		...
		static std::vector<std::string> getAllDiminishedChords() {
			static std::vector<std::string> _alldimchords;
		...
		static std::vector<std::string> getAllSuspendedChords() {
			static std::vector<std::string> _allsuschords;
		...
		*/
		static bool isSlashChord(std::string _name) {
			/*

			std::vector<std::string> slash = utils::splitString(_name,"/'");
			if(slash.size()==0){
				return false;
			}else{
				!isdigit(slash[1][0])
			...
			boost::regex rex{ "\/[a-gA-G]" };//find accidentals
			boost::smatch match;
			...
			{
				//ofLogError()<< "The std::string "<<_name<<" is not a slash chord"<<std::endl;
				return false;
			}
		}
...
//===================================================================
	...
	*/
		...
		static std::shared_ptr<Chord> triad(NotePtr note, NotePtr key) {
			std::shared_ptr<Chord> chord = Chord::create();
			chord->notes.clear();
			chord->name = "";
			chord->setRoot(note);
		...
		*/
		...
		static std::vector<std::shared_ptr<Chord>> triads(NotePtr key) {
			if (_triads_cache[key->name].size() > 0) {
				return Chord::copyCache(_triads_cache[key->name]);
			}

			std::deque<NotePtr> triads = Diatonic::getNotes(key);

			std::vector<std::shared_ptr<Chord>> chords;
			for (int i = 0; i < triads.size(); i++) {
				std::shared_ptr<Chord> chord = Chord::triad(triads[i], key);
		...
		static std::shared_ptr<Chord> majorTriad(NotePtr note) {
			std::shared_ptr<Chord> chord = Chord::create();
			chord->notes.clear();
			chord->name = "M";
			chord->setRoot(note);
		 ...
		 */
		static std::shared_ptr<Chord> minorTriad(NotePtr note) {
			std::shared_ptr<Chord> chord = Chord::create();
			chord->notes.clear();
			chord->name = "m";
			chord->setRoot(note);
		 ...
		 */
		static std::shared_ptr<Chord> diminishedTriad(NotePtr note) {
			std::shared_ptr<Chord> chord = Chord::create();
			chord->notes.clear();
			chord->name = "dim";
			chord->setRoot(note);
		 ...
		 */
		static std::shared_ptr<Chord> augmentedTriad(NotePtr note) {
			std::shared_ptr<Chord> chord = Chord::create();
			chord->notes.clear();
			chord->name = "aug";
			chord->setRoot(note);
...
//===================================================================
	 ...
	 */
		...
		static std::shared_ptr<Chord> seventh(NotePtr note, NotePtr key) {
			std::shared_ptr<Chord> chord = Chord::triad(note, key);
		...
		static std::vector<std::shared_ptr<Chord>> sevenths(NotePtr key) {
			...
			std::vector<std::shared_ptr<Chord>> chords;
			for (int i = 0; i < triads.size(); i++) {
				std::shared_ptr<Chord> chord = Chord::seventh(triads[i], key);
				chords.push_back(chord);
			}
		...
		static std::vector<std::shared_ptr<Chord>> copyCache(std::vector<std::shared_ptr<Chord>>cache) {
			std::vector<std::shared_ptr<Chord>> _copy_cache;
			for (std::shared_ptr<Chord> chord : cache) {
		...
		static std::shared_ptr<Chord> majorSeventh(NotePtr note) {
			std::shared_ptr<Chord> chord = Chord::majorTriad(note);
			chord->name = "M7";
			chord->setRoot(note);
		...
		static std::shared_ptr<Chord> minorSeventh(NotePtr note) {
			std::shared_ptr<Chord> chord = Chord::minorTriad(note);
			chord->name = "m7";
			chord->setRoot(note);
		...
		static std::shared_ptr<Chord> dominantSeventh(NotePtr note) {
			std::shared_ptr<Chord> chord = Chord::majorTriad(note);
			chord->name = "7";
			chord->setRoot(note);
			NotePtr n = Interval::minorSeventh(note);
		...
		static std::shared_ptr<Chord> halfDiminishedSeventh(NotePtr note) {
			std::shared_ptr<Chord> chord = Chord::diminishedTriad(note);
			chord->name = "m7b5";
			chord->setRoot(note);
			NotePtr n = Interval::minorSeventh(note);
			chord->notes.push_back(n);
		...
		static std::shared_ptr<Chord> halfDiminishedNinth(NotePtr note) {
			std::shared_ptr<Chord> chord = Chord::diminishedTriad(note);
			chord->name = "m9b5";
			chord->setRoot(note);
			NotePtr n = Interval::minorSeventh(note);
		...
		static std::shared_ptr<Chord> halfDiminishedEleventh(NotePtr note) {
			std::shared_ptr<Chord> chord = Chord::diminishedTriad(note);
			chord->name = "m11b5";
			chord->setRoot(note);
			NotePtr n = Interval::minorSeventh(note);
		...
		*/
		...
		static std::shared_ptr<Chord> minorSeventhFlatFive(NotePtr note) {
			return halfDiminishedSeventh(note);
		}

		static std::shared_ptr<Chord> minorSeventhFlatNinth(NotePtr note) {
			std::shared_ptr<Chord> chord = Chord::minorNinth(note);
			chord->name = "m7b9";
			chord->setRoot(note);
		...
		static std::shared_ptr<Chord> diminishedSeventh(NotePtr note) {
			std::shared_ptr<Chord> chord = Chord::diminishedTriad(note);
			chord->name = "dim7";
			chord->setRoot(note);
			NotePtr n = Interval::minorSeventh(note);
		...
		static std::shared_ptr<Chord> minorMajorSeventh(NotePtr note) {
			std::shared_ptr<Chord> chord = Chord::minorTriad(note);
			chord->name = "mM7";
			chord->setRoot(note);
			NotePtr n = Interval::majorSeventh(note);
...
//===================================================================
	 ...
	 */
		...
		static std::shared_ptr<Chord> minorSixth(NotePtr note) {
			std::shared_ptr<Chord> chord = Chord::minorTriad(note);
			chord->name = "m6";
			chord->setRoot(note);
		...
		static std::shared_ptr<Chord> majorSixth(NotePtr note) {
			std::shared_ptr<Chord> chord = Chord::majorTriad(note);
			chord->name = "M6";
			chord->setRoot(note);
			NotePtr n = Interval::majorSixth(note);
		...
		static std::shared_ptr<Chord> dominantSixth(NotePtr note) {
			std::shared_ptr<Chord> chord = Chord::majorSixth(note);
			chord->name = "67";
			chord->setRoot(note);
			NotePtr n = Interval::minorSeventh(note);
			chord->notes.push_back(n);
		...
		static std::shared_ptr<Chord> sixthNith(NotePtr note) {
			std::shared_ptr<Chord> chord = Chord::majorSixth(note);
			chord->name = "69";
			chord->setRoot(note);
...
//===================================================================
	 ...
	 */
		...
		static std::shared_ptr<Chord> minorNinth(NotePtr note) {
			std::shared_ptr<Chord> chord = Chord::minorSeventh(note);
			chord->name = "m9";
			chord->setRoot(note);
			NotePtr n = Interval::majorSecond(note);
			n->changeOctave(1);
		...
		static std::shared_ptr<Chord> minorSixthNinth(NotePtr note) {
			std::shared_ptr<Chord> chord = Chord::minorTriad(note);
			chord->name = "m6/9";
			chord->setRoot(note);
			NotePtr n = Interval::majorSixth(note);
		...
		static std::shared_ptr<Chord> majorNinth(NotePtr note) {
			std::shared_ptr<Chord> chord = Chord::majorSeventh(note);
			chord->name = "M9";
			chord->setRoot(note);
			NotePtr n = Interval::majorSecond(note);
			n->changeOctave(1);
		...
		static std::shared_ptr<Chord> dominantNinth(NotePtr note) {
			std::shared_ptr<Chord> chord = Chord::dominantSeventh(note);
			chord->name = "9";
			chord->setRoot(note);
			NotePtr n = Interval::majorSecond(note);
			n->changeOctave(1);
		...
		static std::shared_ptr<Chord> dominantFlatNinth(NotePtr note) {
			std::shared_ptr<Chord> chord = Chord::dominantNinth(note);
			chord->name = "7b9";
			chord->setRoot(note);
		...
		7b9b5
		*/
		static std::shared_ptr<Chord> dominantFlatNinthFlatFifth(NotePtr note) {
			std::shared_ptr<Chord> chord = Chord::dominantNinth(note);
			chord->name = "7b9b5";
			chord->setRoot(note);
		...
		static std::shared_ptr<Chord> dominantSharpNinth(NotePtr note) {
			std::shared_ptr<Chord> chord = Chord::dominantNinth(note);
			chord->name = "7#9";
			chord->setRoot(note);
			NotePtr n = Interval::majorSecond(note);
...
//===================================================================
	 ...
	 */
		...
		static std::shared_ptr<Chord> eleventh(NotePtr note) {
			std::shared_ptr<Chord> chord = Chord::create();
			chord->notes.clear();
			chord->name = "11";
			chord->setRoot(note);
		...
		static std::shared_ptr<Chord> minorEleventh(NotePtr note) {
			std::shared_ptr<Chord> chord = Chord::minorSeventh(note);
			chord->name = "m11";
			chord->setRoot(note);
		...
		static std::shared_ptr<Chord> minorNinthEleventh(NotePtr note) {
			std::shared_ptr<Chord> chord = Chord::minorNinth(note);
			chord->name = "m9/11";
			chord->setRoot(note);
			NotePtr n = Interval::perfectFourth(note);
		...
		static std::shared_ptr<Chord> minorSixthNinthEleventh(NotePtr note) {
			std::shared_ptr<Chord> chord = Chord::minorTriad(note);
			chord->name = "m6/9/11";
			chord->setRoot(note);
			NotePtr n = Interval::majorSixth(note);
		...
		static std::shared_ptr<Chord> majorSeventhSharpEleventh(NotePtr note) {
			std::shared_ptr<Chord> chord = Chord::lydianMajorSeventh(note);
			return chord;
		}
...
//===================================================================
	 ...
	 */
		...
		static std::shared_ptr<Chord> minorThirteenth(NotePtr note) {
			std::shared_ptr<Chord> chord = Chord::minorNinth(note);
			chord->name = "m13";
			chord->setRoot(note);
			NotePtr n = Interval::majorSixth(note);
		...
		static std::shared_ptr<Chord> majorThirteenth(NotePtr note) {
			std::shared_ptr<Chord> chord = Chord::majorNinth(note);
			chord->name = "M13";
			chord->setRoot(note);
			NotePtr n = Interval::majorSixth(note);
			n->changeOctave(1);
		...
		static std::shared_ptr<Chord> dominantThirteenth(NotePtr note) {
			std::shared_ptr<Chord> chord = Chord::dominantNinth(note);
			chord->name = "13";
			chord->setRoot(note);
			NotePtr n = Interval::majorSixth(note);
...
//===================================================================
	 ...
	 */
		...
		static std::shared_ptr<Chord> suspendedTriad(NotePtr note) {
			std::shared_ptr<Chord> chord = Chord::suspendedFourthTriad(note);
			return chord;
		}
		...
		static std::shared_ptr<Chord> suspendedSecondTriad(NotePtr note) {
			std::shared_ptr<Chord> chord = Chord::create();
			chord->notes.clear();
			chord->name = "sus2";
			chord->setRoot(note);
		...
		static std::shared_ptr<Chord> suspendedFourthTriad(NotePtr note) {
			std::shared_ptr<Chord> chord = Chord::create();
			chord->notes.clear();
			chord->name = "sus4";
			chord->setRoot(note);
		...
		static std::shared_ptr<Chord> suspendedSeventh(NotePtr note) {
			std::shared_ptr<Chord> chord = Chord::suspendedFourthTriad(note);
			chord->name = "sus47";
			chord->setRoot(note);
			chord->notes.push_back(note);
		...
		static std::shared_ptr<Chord> suspendedFourthFlatNinth(NotePtr note) {
			std::shared_ptr<Chord> chord = Chord::suspendedFourthTriad(note);
			chord->name = "sus4b9";
			chord->setRoot(note);
			chord->notes.push_back(note);
		...
		static std::shared_ptr<Chord> suspendedNinth(NotePtr note) {
			std::shared_ptr<Chord> chord = Chord::suspendedSeventh(note);
			chord->name = "sus9";
			chord->setRoot(note);
			chord->notes.push_back(note);
			NotePtr n = Interval::majorSecond(note);
...
//===================================================================
	 ...
	 */
		...
		static std::shared_ptr<Chord> augmentedMajorSeventh(NotePtr note) {
			std::shared_ptr<Chord> chord = Chord::augmentedTriad(note);
			chord->name = "M7+5";
			chord->setRoot(note);
			NotePtr n = Interval::majorSeventh(note);
			chord->notes.push_back(n);
		...
		static std::shared_ptr<Chord> augmentedMinorSeventh(NotePtr note) {
			std::shared_ptr<Chord> chord = Chord::minorTriad(note);
			chord->name = "m7+5";
			chord->setRoot(note);
		...
		static std::shared_ptr<Chord> augmentedDominantSeventh(NotePtr note) {
			std::shared_ptr<Chord> chord = Chord::augmentedTriad(note);
			chord->name = "7+5";
			chord->setRoot(note);
			NotePtr n = Interval::minorSeventh(note);
		...
		static std::shared_ptr<Chord> augmentedDominantSharpNinth(NotePtr note) {
			std::shared_ptr<Chord> chord = Chord::dominantSharpNinth(note);
			chord->name = "7+#9";
			chord->setRoot(note);
			chord->notes[2]->augment();
			return chord;
		...
		static std::shared_ptr<Chord> augmentedDominantFlatNinth(NotePtr note) {
			std::shared_ptr<Chord> chord = Chord::dominantFlatNinth(note);
			chord->name = "7+b9";
			chord->setRoot(note);
			chord->notes[2]->augment();
			return chord;
...
//===================================================================
	 ...
	 */
		...
		static std::shared_ptr<Chord> dominantFlatFive(NotePtr note) {
			std::shared_ptr<Chord> chord = Chord::dominantSeventh(note);
			chord->name = "7b5";
			chord->setRoot(note);
			chord->notes[2]->diminish();
		...
		static std::shared_ptr<Chord> dominantSharpNineFlatThirteen(NotePtr note) {
			std::shared_ptr<Chord> chord = Chord::dominantSeventh(note);
			chord->name = "7#9b13";
			chord->setRoot(note);
		...
		static std::shared_ptr<Chord> powerChord(NotePtr note) {
			std::shared_ptr<Chord> chord = Chord::create();
			chord->name = "5";
			chord->notes.clear();
			chord->setRoot(note);
		...
		static std::shared_ptr<Chord> lydianDominantSeventh(NotePtr note) {
			std::shared_ptr<Chord> chord = Chord::dominantSeventh(note);
			chord->name = "7#11";
			chord->setRoot(note);
			NotePtr n = Interval::perfectFourth(note);
		...
		static std::shared_ptr<Chord> lydianDominantNinth(NotePtr note) {
			std::shared_ptr<Chord> chord = Chord::dominantSeventh(note);
			chord->name = "9#11";
			chord->setRoot(note);
			NotePtr n = Interval::majorSecond(note);
		...
		static std::shared_ptr<Chord> lydianMajorSeventh(NotePtr note) {
			std::shared_ptr<Chord> chord = Chord::majorSeventh(note);
			chord->name = "M7#11";
			chord->setRoot(note);
			NotePtr n = Interval::perfectFourth(note);
		...
		static std::shared_ptr<Chord> hendrixChord(NotePtr note) {
			std::shared_ptr<Chord> chord = Chord::dominantSeventh(note);
			chord->name = "hendrix";
			chord->setRoot(note);
			NotePtr n = Interval::minorThird(note);
...
//===================================================================
		...
		static std::shared_ptr<Chord> rootless(std::shared_ptr<Chord> chord) {
			if (!chord->isValid()) 
			{
				return nullptr;
			}
			std::shared_ptr<Chord> chordCopy = chord->copy();
			chordCopy->notes.pop_front();
			return chordCopy;
		}


		static std::shared_ptr<Chord> noThird(std::shared_ptr<Chord> chord) {
			if (!chord->isValid())
			{
				return nullptr;
			}
		...
		static std::shared_ptr<Chord> noFifth(std::shared_ptr<Chord> chord) {
			if (!chord->isValid())
			{
				return nullptr;
			}
...
//===================================================================
	 ...
	 */
		...
		static std::shared_ptr<Chord> tonic(NotePtr key) {
		...
		static std::shared_ptr<Chord> tonic7(NotePtr key) {
			std::shared_ptr<Chord> chord = Chord::sevenths(key)[0];
			return chord;
		}
		...
		static std::shared_ptr<Chord> supertonic(NotePtr key) {
		...
		static std::shared_ptr<Chord> supertonic7(NotePtr key) {
			std::shared_ptr<Chord> chord = Chord::sevenths(key)[1];
			return chord;
		}
		...
		static std::shared_ptr<Chord> mediant(NotePtr key) {
		...
		static std::shared_ptr<Chord> mediant7(NotePtr key) {
			std::shared_ptr<Chord> chord = Chord::sevenths(key)[2];
		...
		static std::shared_ptr<Chord> subdominant(NotePtr key) {
		...
		static std::shared_ptr<Chord> subdominant7(NotePtr key) {
			std::shared_ptr<Chord> chord = Chord::sevenths(key)[3];
			//chord->name += "7";
			return chord;
		...
		static std::shared_ptr<Chord> dominant(NotePtr key) {
		...
		static std::shared_ptr<Chord> dominant7(NotePtr key) {
			std::shared_ptr<Chord> chord = Chord::sevenths(key)[4];
			//chord->name += "7";
			return chord;
		...
		static std::shared_ptr<Chord> submediant(NotePtr key) {
		...
		static std::shared_ptr<Chord> submediant7(NotePtr key) {
			std::shared_ptr<Chord> chord = Chord::sevenths(key)[5];
			//chord->name += "7";
			return chord;
		}
		...
		static std::shared_ptr<Chord> subtonic(NotePtr key) {
			NotePtr wholeNoteDown = key->getTransposed(-2);
			std::shared_ptr<Chord> ch = Chord::majorTriad(wholeNoteDown);
			ch->setName("");
			return ch;
		...
		static std::shared_ptr<Chord> subtonic7(NotePtr key) {

			NotePtr wholeNoteDown = key->getTransposed(-2);
			std::shared_ptr<Chord> ch = Chord::dominantSeventh(wholeNoteDown);
			ch->setName("7");
			return ch;
		...
		static std::shared_ptr<Chord> leadingtone(NotePtr key) {
			return Chord::triads(key)[6];
		}
		...
		static std::shared_ptr<Chord> leadingtone7(NotePtr key) {
			std::shared_ptr<Chord> chord = Chord::sevenths(key)[6];
			return chord;
		}
...
//===================================================================
		...
		static std::shared_ptr<Chord> I(NotePtr key) {
		...
		https://www.reddit.com/r/musictheory/comments/1bdws7/i7_in_g_major_uses_f_or_f/
		 ...
		 this was
		static std::shared_ptr<Chord> I7(NotePtr key){
		...
		*/
		static std::shared_ptr<Chord> IM7(NotePtr key) {
		...
		static std::shared_ptr<Chord> ii(NotePtr key) {
		...
		static std::shared_ptr<Chord> ii7(NotePtr key) {
		...
		static std::shared_ptr<Chord> iii(NotePtr key) {
		...
		static std::shared_ptr<Chord> iii7(NotePtr key) {
		...
		static std::shared_ptr<Chord> IV(NotePtr key) {
			return Chord::subdominant(key);
		}

		static std::shared_ptr<Chord> IV7(NotePtr key) {
			NotePtr transKey = Interval::fourth(key, key);
			return Chord::dominantSeventh(transKey);
			//return Chord::subdominant7(key);
		}

		static std::shared_ptr<Chord> V(NotePtr key) {
			return Chord::dominant(key);
		}

		static std::shared_ptr<Chord> V7(NotePtr key) {
			return Chord::dominant7(key);
		}

		static std::shared_ptr<Chord> vi(NotePtr key) {
		...
		static std::shared_ptr<Chord> vi7(NotePtr key) {
		...
		static std::shared_ptr<Chord> bvii(NotePtr key) {
		...
		static std::shared_ptr<Chord> vii(NotePtr key) {
		...
		static std::shared_ptr<Chord> bvii7(NotePtr key) {
		...
		static std::shared_ptr<Chord> vii7(NotePtr key) {
...
//===================================================================
	...
	*/
		...
		static std::shared_ptr<Chord> rootPosition(std::shared_ptr<Chord> chord) {
			std::shared_ptr<Chord> newCopy = Chord::create(chord->getName());
			return newCopy;
		}

		std::shared_ptr<Chord> rootPosition() {
			std::shared_ptr<Chord> newCopy = Chord::create(getName());
			return newCopy;
		}
		...
		This return a copy
		*/
		std::shared_ptr<Chord> invert() {
			std::shared_ptr<Chord> c = copy();
		...
		/*
		 ...
		 Returns a copy
		 */
		static std::shared_ptr<Chord> invert(std::shared_ptr<Chord> chord) {
			std::shared_ptr<Chord> inv = chord->copy();
		 ...
		 */
		static void invert(std::deque<NotePtr>& notes) {
			if (notes.size() > 1) {
		...
		*/
		...
		static std::shared_ptr<Chord> firstInversion(std::shared_ptr<Chord> chord) {
		 ...
		 */
		static std::shared_ptr<Chord> secondInversion(std::shared_ptr<Chord> chord) {
		 ...
		 */
		static std::shared_ptr<Chord> thirdInversion(std::shared_ptr<Chord> chord) {
		 ...
		 */
		static std::shared_ptr<Chord> fourthInversion(std::shared_ptr<Chord> chord) {
		 ...
		 */
		static std::shared_ptr<Chord> fifthInversion(std::shared_ptr<Chord> chord) {
...
//===================================================================
   ...
   */
		...
		static std::shared_ptr<Chord> findNearestVoicing(std::shared_ptr<Chord> thisChord, std::shared_ptr<Chord> chordToTransform) {
			...
			std::shared_ptr<Chord>chordCopy = chordToTransform->copy();
			std::deque<NotePtr>orgNotes = chordCopy->notes;
		...
		*/
		...
		static std::shared_ptr<Chord> findNearestVoicing(std::shared_ptr<Chord> chordToTransform, std::shared_ptr<Note> note) {
			if (!chordToTransform->isValid()) {
				return chordToTransform;
			}
			int mid = note->getInt();;
			std::shared_ptr<Note> ref = Note::fromInt(mid);
			std::shared_ptr<Chord>chordCopy = chordToTransform->copy();
			std::deque<NotePtr>orgNotes = chordCopy->notes;
			chordCopy->notes.clear();
		...
		static std::shared_ptr<Chord> findNearestVoicingBelow(std::shared_ptr<Chord> chordToTransform, std::shared_ptr<Note> note, bool okToOverlap = true) {
			if (!chordToTransform->isValid()) {
				return chordToTransform;
			}
			int mid = note->getInt();;
		...
		static std::shared_ptr<Chord> findNearestVoicingAbove(std::shared_ptr<Chord> chordToTransform, std::shared_ptr<Note> note, bool okToOverlap = true) {
			if (!chordToTransform->isValid()) {
				return chordToTransform;
			}
			int mid = note->getInt();
		...
		*/
		...
		static std::shared_ptr<Chord> findNearestTransposition(std::shared_ptr<Chord> chordToTransform, std::shared_ptr<Note> note) {
			if (!chordToTransform->isValid()) {
				return chordToTransform;
			}
			...
			if (currDist > 0) {
				...
				//below, pitch up
				int octUpDist = std::abs((mid + 12) - centre);
				bool cont = true;
				while (octUpDist < currDist && cont) {
					currDist = mid - centre;
					chordCopy->octaveUp();
					mid = (chordCopy->notes.front()->getInt() + chordCopy->notes.back()->getInt()) / 2;
					octUpDist = std::abs(mid - centre);
					if (octUpDist >= currDist) {
						//doesn't make a difference or same
						chordCopy->octaveDown();
						cont = false;
		...
		static void sortNotesOnPitch(std::shared_ptr<Chord>chord) {
...
//===================================================================
		...
		static std::vector<std::string> determineDiad(std::deque<NotePtr> diad, bool shorthand = false, bool allowInversions = true) {
			...
			std::string intervalName = Interval::determine(diad[0], diad[1], shorthand);
		...
		static std::vector<std::string> determineTriad(std::deque<NotePtr> triad, bool shorthand = false, bool allowInversions = true) {
		...
		static std::vector<std::string>  determineSeventh(std::deque<NotePtr> chord, bool shorthand = false, bool allowInversions = true, bool allowPolychords = true) {
			...
			if (chord.size() != 4) {
#ifdef LOGS
				ofLog() << "warning raise exception: seventh chord is not a seventh chord" << std::endl;
#endif // LOGS
				return inversions;
			}
		...
		static std::vector<std::string>  determineExtended5Chord(std::deque<NotePtr> chord, bool shorthand = false, bool allowInversions = true, bool allowPolychords = true) {
			...
			if (chord.size() != 5) {
#ifdef LOGS
				ofLog() << "warning raise exeption: not an extended chord" << std::endl;
#endif // LOGS
				return inversions;
			}
		...
		static std::vector<std::string>  determineExtended6Chord(std::deque<NotePtr> chord, bool shorthand = false, bool allowInversions = true, bool allowPolychords = true) {
		...
		static std::vector<std::string>  determineExtended7Chord(std::deque<NotePtr> chord, bool shorthand = false, bool allowInversions = true, bool allowPolychords = true) {
		...
		static std::vector<std::string>  determinePolychords(std::deque<NotePtr> chord, bool shorthand = false, bool allowInversions = true) {
...
//===================================================================
	 ...
	 */
		...
		static std::vector<std::string>  determine(std::deque<NotePtr> chord, bool shorthand = false, bool allowInversions = true, bool allowPolychords = false) {
			//cout<<"Chord::determine"<<std::endl;
		...
		std::vector<std::string>  determine(std::vector<std::string> chords, bool shorthand = false, bool no_inversions = false, bool no_polychords = false) {
		}
		/*
		std::vector<std::string> determine(chord, bool shorthand=false, bool no_inversions=false, bool no_polychords=false){
		}
		  */
		...
		static std::vector<std::string>  analyse(std::deque<NotePtr> chord, bool shorthand = false, bool allowInversions = true, bool allowPolychords = false) {
		...
		static std::string getRootNote(std::string chordname) {
			std::string root = chordname.substr(0, 1);
		 ...
		 */
		static std::string getChordSymbol(std::string chordname) {
			if (chordname.size() == 0) {
				return "";
			}
			//cout<<"Chord::getChordSymbol "<<chordname<<std::endl;
			auto split = utils::splitString(chordname, "/");
			chordname = split[0];//if different bass
			int numberOfAccidentals = utils::getNumberOfAccidentals(chordname);
			if ((chordname.size() - numberOfAccidentals) == 1) {
				return "";//just triad
			}
			std::string sign;
...
//===================================================================
		...
		static std::shared_ptr<Chord> chordFromShorthand(std::string c, NotePtr note) {
...
#ifdef LOGS
				ofLog() << "Warning: Chord abbr " << note->getName() << " " << c << " not recognized" << std::endl;
#endif // LOGS
				return 0;
			}
...
//===================================================================
	...
	private:
		...
		static std::vector<std::string> splitChord(std::string chordString)
		{
			return utils::splitString(chordString, ",");
		}
		...
		static void triadInversionExhauster(std::deque<NotePtr> triad, bool shorthand, int tries, std::vector<std::string>* result, bool allowInversions) {
			...
			/*
			 ...
			 std::string intval1 = Interval::determine(C, C#, true);//#1
			 std::string intval2 = Interval::determine(C, Db, true);//b2
			...
			if (int1 == 2 && int2 == 7) {
				result->push_back("sus2," + std::to_string(tries) + "," + triad[0]->name);
			}
			else if (int1 == 4 && int2 == 10) {
				result->push_back("dom7," + std::to_string(tries) + "," + triad[0]->name);
			}
			else if (int1 == 4 && int2 == 6) {
				result->push_back("7b5," + std::to_string(tries) + "," + triad[0]->name);
			}
			else if (int1 == 4 && int2 == 7) {
				result->push_back("M," + std::to_string(tries) + "," + triad[0]->name);
			}
			else if (int1 == 4 && int2 == 8) {
				result->push_back("aug," + std::to_string(tries) + "," + triad[0]->name);
			}
			else if (int1 == 4 && int2 == 4) {
				result->push_back("M6," + std::to_string(tries) + "," + triad[0]->name);
			}
			else if (int1 == 4 && int2 == 11) {
				result->push_back("M7," + std::to_string(tries) + "," + triad[0]->name);
			}
			else if (int1 == 3 && int2 == 6) {
				result->push_back("dim," + std::to_string(tries) + "," + triad[0]->name);
			}
				...
				result->push_back("m/M7," + std::to_string(tries) + "," + triad[0]->name);
			}
			else if (int1 == 5 && int2 == 7) {
				result->push_back("sus4," + std::to_string(tries) + "," + triad[0]->name);
			}
			...
			if (tries != 3 && allowInversions) {
				...
				for (int r = 0; r < rearrange; r++) {
					auto split = splitChord((*result)[r]);
					if (split.size() != 3) {
						return;
					}
					if (shorthand) {
		...
		static void seventhInversionExhauster(std::deque<NotePtr> seventh, bool shorthand, int tries, std::vector<std::string>* result, bool allowInversions) {
			// Check whether the first three notes of seventh
			//are part of some triad.
			std::deque<NotePtr> sub;
			sub.insert(sub.begin(), seventh.begin(), seventh.begin() + 3);

			//not sure why no inversions here
			std::vector<std::string> triads = Chord::determineTriad(sub, true, allowInversions);
			...
			for (int i = 0; i < triads.size(); i++) {
				//Basic triads
				//std::string triadStr = triads[i].substr(seventh[0].name.size());

				std::string chStr = Chord::getChordSymbol(triads[i]);
				std::string root = Chord::getRootNote(triads[i]);

				//Get the interval between the first and last note
				std::string intval3 = Interval::determine(Note::create(root), seventh[3]);
				...
				if (chStr == "m") {
					if (int1 == 10) {
						result->push_back("m7," + std::to_string(tries) + "," + root);
					}
					else if (int1 == 11) {
						result->push_back("m/M7," + std::to_string(tries) + "," + root);
					}
					else if (int1 == 9) {
						result->push_back("m6," + std::to_string(tries) + "," + root);
					}
				}
				else if (chStr == "M") {
					...
					if (int1 == 11) {
						...
						result->push_back("7," + std::to_string(tries) + "," + root);
					}
					else if (int1 == 9) {
						result->push_back("M6," + std::to_string(tries) + "," + root);
					}
					...
					if (int1 == 10) {
						result->push_back("m7b5," + std::to_string(tries) + "," + root);
					}
					else if (intval3 == "diminished seventh" || intval3 == "major sixth") {
						//not sure how to make this int
						result->push_back("dim7," + std::to_string(tries) + "," + root);
					}
				}
				else if (chStr == "aug") {
					...
					if (int1 == 10) {
						result->push_back("sus47," + std::to_string(tries) + "," + root);
					}
					else if (int1 == 1) {
						result->push_back("sus4b9," + std::to_string(tries) + "," + root);
					}
					else if (int1 == 2) {
						result->push_back("sus9," + std::to_string(tries) + "," + root);
					}
					//Other
				}
				else if (chStr == "m7") {
					if (int1 == 5) {
						result->push_back("11," + std::to_string(tries) + "," + root);
					}
				}
				else if (chStr == "7b5") {
					if (int1 == 10) {
						result->push_back("7b5," + std::to_string(tries) + "," + root);
					}
				}
			...
			if (tries != 4 && allowInversions) {
				...
				// cout<<"No inversion for seventh found"<<std::endl;
				return;
			   ...
			   // seventh = [seventh[3]] + seventh[0:3];
				...
				for (int r = 0; r < rearrange; r++) {
					auto split = splitChord((*result)[r]);
					if (split.size() != 3) {
						// cout<<"No inversion for seventh found 2"<<std::endl;
						return;
					}
					if (shorthand) {
						(*result)[r] = split[2] + split[0];
		...
		static void chord5InversionExhauster(std::deque<NotePtr> chord, bool shorthand, int tries, std::vector<std::string>* result, bool allowInversions) {


			std::deque<NotePtr> sub;
			sub.insert(sub.begin(), chord.begin(), chord.begin() + 3);
			std::vector<std::string> triads = Chord::determineTriad(sub, true, true);


			sub.push_back(chord[3]);
			std::vector<std::string> sevenths = Chord::determineSeventh(sub, true, allowInversions, false);//polychords called outside
			...
			for (int s = 0; s < sevenths.size(); s++) {
				...
				//std::string seventh = sevenths[s].substr(chord[0].name.size());

				std::string chStr = Chord::getChordSymbol(sevenths[s]);
				std::string root = Chord::getRootNote(sevenths[s]);


				//Get the interval between the first and last note
				std::string intval4 = Interval::determine(Note::create(root), chord[4]);
				int int4 = Interval::measure(Note::create(root), chord[4]);

				if (chStr == "M7") {
					if (int4 == 2) {
						result->push_back("M9," + std::to_string(tries) + "," + root);
					}
				}
				else if (chStr == "m7") {
					if (int4 == 2) {
						...
						result->push_back("m11," + std::to_string(tries) + "," + root);
					}
				}
				else if (chStr == "7") {
					if (int4 == 2) {
						result->push_back("9," + std::to_string(tries) + "," + root);
					}
					else if (int4 == 1) {
						result->push_back("7b9," + std::to_string(tries) + "," + root);
					}
					else if (int4 == 3) {
						result->push_back("7#9," + std::to_string(tries) + "," + root);
					}
					else if (int4 == 3) {
						result->push_back("7b12," + std::to_string(tries) + "," + root);
					}
					else if (int4 == 6) {
						result->push_back("7#11," + std::to_string(tries) + "," + root);
					}
					else if (int4 == 9) {
						result->push_back("13," + std::to_string(tries) + "," + root);
					}
				}
				else if (chStr == "m7+") {
					if (int4 == 3) {
					...
					if (int4 == 2) {
						result->push_back("6/9," + std::to_string(tries) + "," + root);
					}
					else if (int4 == 11) {
						result->push_back("6/7," + std::to_string(tries) + "," + root);
					}
				}
			}
		...
		static void chord6InversionExhauster(std::deque<NotePtr> chord, bool shorthand, int tries, std::vector<std::string>* result, bool allowInversions) {


			std::deque<NotePtr> sub;
			sub.insert(sub.begin(), chord.begin(), chord.begin() + 5);
			std::vector<std::string> ch = Chord::determineExtended5Chord(sub, true, allowInversions, false);



			for (int c = 0; c < ch.size(); c++) {
				std::string ochStr = ch[c].substr(chord[0]->name.size());

				std::string chStr = Chord::getChordSymbol(ch[c]);
				std::string root = Chord::getRootNote(ch[c]);


				//Get the interval between the first and last note
				std::string intval5 = Interval::determine(Note::create(root), chord[5]);
				...
				if (chStr == "9") {
					...
					if (int5 == 5) {
						...
						result->push_back("m13," + std::to_string(tries) + "," + root);
					}
				}
				else if (chStr == "M9") {
					if (int5 == 5) {
		...
		static void chord7InversionExhauster(std::deque<NotePtr> chord, bool shorthand, int tries, std::vector<std::string>* result, bool allowInversions) {


			std::deque<NotePtr> sub;
			sub.insert(sub.cbegin(), chord.begin(), chord.begin() + 6);
			std::vector<std::string> ch = Chord::determineExtended6Chord(sub, true, true, false);
			...
			for (int c = 0; c < ch.size(); c++) {
				...
				//Get the interval between the first and last note
				std::string intval6 = Interval::determine(Note::create(root), chord[6]);
				...
				if (chStr == "11") {
					...
					//cout<<"--------m13, "<<chord[0]<<"  "<<intval6<<std::endl;
					if (int6 == 9) {
						result->push_back("m13," + std::to_string(tries) + "," + root);
					}
				}
		...
		static void polychordExhauster(std::deque<NotePtr> chord, bool shorthand, int tries, std::vector<std::string>* result, bool allowInversions) {
			...
			if (chord.size() <= 3) {
#ifdef LOGS
				ofLog() << "No polychord with less than 3 notes" << std::endl;
#endif // LOGS
				return;
			}
			else if (chord.size() > 14) {
#ifdef LOGS
				ofLog() << "A polychord with more than 14 notes?! You kidding?" << std::endl;
#endif // LOGS
				return;
			}
			...
			for (int f = 0; f < function_nr; f++) {
				...
				std::vector<std::string> chord1options;

				//build new inversion
				std::deque<NotePtr> chord1;
				// cout<<chord.size() -(3 + f)<<std::endl;
				chord1.insert(chord1.cbegin(), chord.begin() + chord.size() - (3 + f), chord.end());
				...
				for (int f2 = 0; f2 < function_nr; f2++) {
					...
					std::vector<std::string> chord2options;

					//build new inversion
					std::deque<NotePtr> chord2;

					chord2.insert(chord2.cbegin(), chord.begin(), chord.begin() + 3 + f2);
		...
		static void subcall(std::deque<NotePtr> chord, bool shorthand, int tries, std::vector<std::string>* result, bool allowInversions) {
		...
		/*
		 Helper function that returns the inversion of the triad in a std::string
		 */

		static std::string int_desc(std::string tries) {
			if (tries == "1") {
				return "";
			}
			else if (tries == "2") {
		...
		friend std::ostream& operator<<(std::ostream& os, const Chord& n);
		friend std::ostream& operator<<(std::ostream& os, const std::shared_ptr<Chord>& n);
	};//class
	...
	inline std::ostream& operator<<(std::ostream& os, Chord& c) {
		if (c.notes.size() == 0) {
			os << "Chord invalid" << std::endl;
		}
		else {
	...
	inline std::ostream& operator<<(std::ostream& os, std::shared_ptr<Chord>& c) {
		if (!Chord::isValid(c)) {
			os << "Chord invalid" << std::endl;
		}
		else {
